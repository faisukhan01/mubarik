        ...
        <div className="container-site h-20 flex items-center justify-between">
          ...
          <Link href="#" className="flex items-center gap-3 flex-shrink-0">
            <Image
              ...
              width={54}
              height={54}
              className="w-[54px] h-[54px] lg:w-[70px] lg:h-[70px] transition-all"
              priority
            />
            <div className="flex flex-col leading-tight">
              <span className="text-lg sm:text-xl lg:text-2xl font-semibold text-navy" style={{ fontFamily: 'var(--font-playfair), Georgia, serif' }}>
                Mubarik
              </span>
              <span className="text-[0.625rem] sm:text-[0.6875rem] text-text-secondary font-medium tracking-wide">
                School & Academy
              </span>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="relative px-3.5 py-2 text-[0.8125rem] font-medium text-text-secondary hover:text-navy transition-colors group"
              >
                {item.label}
                <span className="absolute bottom-0 left-3.5 right-3.5 h-[1.5px] bg-navy scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-200" />
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-4">
            <Link
              href="#admissions"
              className="hidden lg:inline-flex items-center px-5 py-2.5 bg-navy text-white text-[0.8125rem] font-semibold tracking-wide hover:bg-navy-light transition-colors rounded-lg"
            >
              Apply for Admission
            </Link>
          ...
          <div className="container-site flex-1">
            ...
            <div className="mt-8">
              <Link
                href="#admissions"
                onClick={() => setMobileOpen(false)}
                className="block w-full text-center px-6 py-3.5 bg-navy text-white text-sm font-semibold tracking-wide hover:bg-navy-light transition-colors rounded-lg"
              >
                Apply for Admission
              </Link>
            </div>
